package org.sergeantKoala.service.comparisionStrategies;

import org.sergeantKoala.service.ComparisonStrategy;

import java.io.IOException;
import java.net.URL;
import java.nio.charset.StandardCharsets;

public class ContentSizeStrategy implements ComparisonStrategy<Integer> {
    @Override
    public Integer extractComparable(String url) {
        try {
            String content = new String(new URL(url).openStream().readAllBytes(), StandardCharsets.UTF_8);
            return content.length();
        } catch (IOException e) {
            throw new RuntimeException("Failed to fetch content for URL: " + url, e);
        }
    }

    @Override
    public boolean isSame(Integer oldValue, Integer newValue) {
        return oldValue.equals(newValue);
    }
}
